            ...
            <div className={`grid grid-cols-1 gap-6 ${isLoading ? 'opacity-50' : ''}`}>
                {isLoading ? (
                    ...
                    filteredPosts.map((post) => (
                        <div key={post.id} className="bg-card border border-border rounded-2xl p-6 flex flex-col md:flex-row gap-6 hover:shadow-lg transition-all group">
                            ...
                            <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-4 text-xs text-muted-foreground mb-2">
                                    <span className="flex items-center gap-1"><Calendar className="h-3 w-3" /> {new Date(post.date).toLocaleDateString('de-DE')}</span>
                                    <span className="flex items-center gap-1"><UserIcon className="h-3 w-3" /> {post.author}</span>
                                </div>
                                <h3 className="text-xl font-bold mb-2 group-hover:text-primary transition-colors truncate">{post.title}</h3>
                                <p className="text-sm text-muted-foreground line-clamp-2 mb-4">{post.excerpt}</p>
                                <div className="flex flex-wrap gap-2">
                                    {post.tags.map(tag => (
                                        <span key={tag} className="text-[10px] px-2 py-0.5 bg-secondary rounded-full border border-border uppercase font-bold tracking-wider">
                                            {tag}
                                        </span>
                                    ))}
                                </div>
                        ...
                        <form onSubmit={handleSave} className="p-8 space-y-8">
                            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                                ...
                                <div className="space-y-8">
                                    ...
                                    <div>
                                        <label className="block text-sm font-bold mb-3">Zusammenfassung (Excerpt)</label>
                                        <textarea
                                            required
                                            rows={4}
